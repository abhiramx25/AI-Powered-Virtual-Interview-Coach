...
# API KEY - Your Groq API key is here
...
class GroqClient:
    def __init__(self, api_key=None):
        ...
        # Priority: 1. Parameter, 2. Variable above, 3. Environment, 4. Streamlit Secrets
        if api_key:
            self.api_key = api_key
        elif GROQ_API_KEY and GROQ_API_KEY != "your_api_key_here":
            self.api_key = GROQ_API_KEY
        else:
            self.api_key = os.getenv("GROQ_API_KEY")
        
        # Last resort: try Streamlit secrets
        if not self.api_key:
            try:
        ...
        if not self.api_key:
            raise ValueError("⚠️ GROQ_API_KEY not found. Please check groq_client.py line 4")
